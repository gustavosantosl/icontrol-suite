...
-- Add check constraints (drop and recreate to avoid conflicts)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'transactions_type_check') THEN
    ALTER TABLE public.transactions ADD CONSTRAINT transactions_type_check CHECK (type IN ('pagar', 'receber'));
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'transactions_status_check') THEN
    ALTER TABLE public.transactions ADD CONSTRAINT transactions_status_check CHECK (status IN ('pending', 'paid', 'overdue'));
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'transactions_total_value_positive') THEN
    ALTER TABLE public.transactions ADD CONSTRAINT transactions_total_value_positive CHECK (total_value > 0);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'transactions_num_installments_positive') THEN
    ALTER TABLE public.transactions ADD CONSTRAINT transactions_num_installments_positive CHECK (num_installments >= 1);
  END IF;
END
$$;
